    ...
    bal = await page.evaluate(
        """
            function sell_price () {
                ...
                let arr = [];
                for(let i = 0; i < 10; i++){
                    arr.push([x[i].innerText, x[i].children[0].href]);
                }
...
async def main_manga():
    ...
    url_list = [
        'https://manhuaplus.com/manga/apotheosis/']
...
if __name__ == "__main__":
    while True:
        ...
        try:
            ...
            output_console = "[" + current_time + "] " + \
                "Starting the search for mangas!\n"
            log = open("log.txt", "a")
            ...
            current_time = str(datetime.datetime.now())
            output_console = "[" + str(current_time) + "] " + \
                str(lst_not_read_dicts) + "\n"
            log = open("log.txt", "a")
            log.write(output_console)
            log.close()
